...
class GoogleAnalytics {
    ...
    static getMachineID() {
        ...
        switch (platform) {
            case "darwin":
                result = child_process_1.execSync("ioreg -rd1 -c IOPlatformExpertDevice").toString()
                    .split("IOPlatformUUID")[1]
                    .split("\n")[0].replace(/\=|\s+|\"/ig, "")
                    .toLowerCase();
                break;
            case "win32":
                result = child_process_1.execSync("REG QUERY HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography /v MachineGuid")
                    .toString()
                    .split("REG_SZ")[1]
                    .replace(/\r+|\n+|\s+/ig, "")
                    .toLowerCase();
            ...
            case "linux":
                result =
                    child_process_1.execSync("( cat /var/lib/dbus/machine-id /etc/machine-id 2> /dev/null || hostname ) | head -n 1 || :")
                        .toString()
                        .replace(/\r+|\n+|\s+/ig, "")
                        .toLowerCase();
                break;
            case "freebsd":
                result = child_process_1.execSync("kenv -q smbios.system.uuid").toString()
                    .replace(/\r+|\n+|\s+/ig, "")
                    .toLowerCase();
                break;
        }
        result = crypto_1.createHash("sha256").update(result).digest("hex");
        return result;
    }
    ...
    static getNpmVersion() {
        if (!this.npmVersion) {
            this.npmVersion = "";
            const buffer = child_process_1.execSync("npm -v");
