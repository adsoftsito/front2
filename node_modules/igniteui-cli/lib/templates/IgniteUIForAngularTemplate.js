...
class IgniteUIForAngularTemplate extends AngularTemplate_1.AngularTemplate {
    ...
    registerInProject(projectPath, name, options) {
        ...
        routingModule.addRoute(path.join(projectPath, `src/app/${this.folderName(name)}/${this.fileName(name)}.component.ts`), this.folderName(name), //path
        name //text
        );
        ...
        for (const dep of this.dependencies) {
            if (dep.from && dep.from.startsWith(".")) {
                ...
                dep.from = TypeScriptUtils_1.TypeScriptUtils.relativePath(mainModulePath, path.join(projectPath, dep.from), true, true);
            }
