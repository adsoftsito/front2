...
class TimeAdapter {
    ...
    static isTimeAvailable(time, min, max, granularity) {
        ...
        const /** @type {?} */ convertedTime = this.convertTimeToMoment(time);
        const /** @type {?} */ isAfter = (min && !max)
            && convertedTime.isSameOrAfter(min, granularity);
        const /** @type {?} */ isBefore = (max && !min)
            && convertedTime.isSameOrBefore(max, granularity);
        const /** @type {?} */ isBetween = (min && max)
            && convertedTime.isBetween(min, max, granularity, '[]');
        const /** @type {?} */ isAvailable = !min && !max;
    ...
    static formatHour(currentHour, format, period) {
        ...
        let /** @type {?} */ hour = period === TimePeriod.AM ? currentHour : currentHour + 12;
...
class NgxMaterialTimepickerService {
    ...
    getFullTime(format) {
        ...
        const /** @type {?} */ period = format === 12 ? this.periodSubject.getValue() : '';
...
(function (TimepickerTime) {
    ...
    function getHours(format) {
        return Array(format).fill(1).map((v, i) => {
            ...
            const /** @type {?} */ time = v + i;
            const /** @type {?} */ angle = angleStep * time;
    ...
    function disableHours(hours, config) {
        if (config.min || config.max) {
            return hours.map(value => {
                const /** @type {?} */ hour = config.format === 24 ? value.time : TimeAdapter.formatHour(+value.time, config.format, config.period);
                const /** @type {?} */ currentTime = moment$2().hour(+hour).format(TimeFormat.TWELVE);
    ...
    function getMinutes(gap = 1) {
        const /** @type {?} */ minutesCount = 60;
        const /** @type {?} */ angleStep = 360 / minutesCount;
        const /** @type {?} */ minutes = [];
        for (let /** @type {?} */ i = 0; i < minutesCount; i++) {
            const /** @type {?} */ angle = angleStep * i;
            if (i % gap === 0) {
    ...
    function disableMinutes(minutes, selectedHour, config) {
        if (config.min || config.max) {
            const /** @type {?} */ hour = TimeAdapter.formatHour(selectedHour, config.format, config.period);
            return minutes.map(value => {
                const /** @type {?} */ currentTime = moment$2().hour(hour).minute(+value.time).format(TimeFormat.TWELVE);
...
class NgxMaterialTimepickerMinutesFaceComponent {
    ...
    ngOnChanges(changes) {
        if (changes['period'] && changes['period'].currentValue) {
            const /** @type {?} */ minutes = TimepickerTime.getMinutes(this.minutesGap);
...
class NgxMaterialTimepickerFaceComponent {
     ...
     */
    ngOnChanges(changes) {
        const /** @type {?} */ faceTimeChanges = changes['faceTime'];
        const /** @type {?} */ selectedTimeChanges = changes['selectedTime'];
...
class TimepickerDirective {
    ...
    set value(value) {
        ...
        const /** @type {?} */ time = TimeAdapter.formatTime(value, this._format);
...
class NgxMaterialTimepickerDialComponent {
    ...
    ngOnChanges(changes) {
        if (changes['period'] && changes['period'].currentValue
            ...
            const /** @type {?} */ hours = TimepickerTime.getHours(this.format);
...
class NgxMaterialTimepickerDialControlComponent {
     ...
     * @return {?}
     */
    saveTimeAndChangeTimeUnit(unit) {
     ...
     */
    updateTime() {
        const /** @type {?} */ time = this.selectedTime;
...
/** @nocollapse */
MinutesFormatterPipe.ctorParameters = () => [];
...
class NgxMaterialTimepickerPeriodComponent {
    ...
    isSwitchPeriodAvailable(period) {
        const /** @type {?} */ time = this.getDisabledTimeByPeriod(period);
...
/** @nocollapse */
NgxMaterialTimepickerPeriodComponent.ctorParameters = () => [];
