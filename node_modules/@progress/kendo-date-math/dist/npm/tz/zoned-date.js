...
var abbr_timezone_1 = require("./abbr-timezone");
var offset_1 = require("./offset");
var to_local_date_1 = require("./to-local-date");
var addMinutes = function (date, minutes) { return new Date(date.getTime() + minutes * constants_1.MS_PER_MINUTE); };
var addHours = function (date, hours) { return new Date(date.getTime() + hours * constants_1.MS_PER_HOUR); };
var dayAbbr = [
...
function isZoneMissingHour(date, timezone) {
    var currentOffset = offset_1.offset(timezone, date);
    var prevHour = addHours(date, -1);
    var prevOffset = offset_1.offset(timezone, prevHour);
...
function convertTimezoneUTC(utcLocal, fromTimezone, toTimezone) {
    ...
    var fromOffset = offset_1.offset(fromTimezone, utcLocal);
    var toOffset = offset_1.offset(toTimezone, utcLocal);
    var baseDiff = fromOffset - toOffset;
    var midDate = addMinutes(utcLocal, baseDiff);
    var midOffset = offset_1.offset(toTimezone, midDate);
...
var ZonedDate = /** @class */ (function () {
    ...
    function ZonedDate(utcDate, timezone) {
        this._utcDate = clone_date_1.cloneDate(utcDate);
        this.timezone = timezone;
        var tzOffset = offset_1.offset(timezone, utcDate);
     ...
     *
     * If the `timezone` parameter is omitted, the `ZonedDate` defaults to the timezone of the browser.
     * This concept is known as "floating date" because it does not represent a particular moment in time.
     * Instead, its actual value depends on the current timezone of the browser.
     *
    ...
    ZonedDate.fromLocalDate = function (date, timezone) {
        ...
        var zoneOffset = offset_1.offset(timezone, utcDate);
        var fixedOffset = 0;
        if (shiftZone) {
    ...
    ZonedDate.prototype.toLocalDate = function () {
        return clone_date_1.cloneDate(this._localDate);
    };
    /**
    ...
    ZonedDate.prototype.toUTCDate = function () {
        return clone_date_1.cloneDate(this._utcDate);
    };
    /**
    ...
    ZonedDate.prototype.toTimezone = function (toTimezone) {
        ...
        var tzOffset = offset_1.offset(this.timezone, this._utcDate);
     ...
     * with the resulting date in the same timezone.
     *
    ...
    /**
     * Adds the specified number of milliseconds and returns a new instance
     * with the resulting date in the same timezone.
     *
     * The method compensates for DST transitions and
     * ensures that the resulting date occurs exactly after the set
     * amount of time in the timezone.
     *
    ...
    /**
     * Returns a new instance of the same zoned date
     * having its time parts set to `00:00:00.000`.
     *
    ...
    ZonedDate.prototype.stripTime = function () {
        ...
        var ticks = Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0);
    ...
    ZonedDate.prototype.toDateString = function () {
        return to_local_date_1.toLocalDate(this._utcDate).toDateString();
    };
    /** @hidden */
    ZonedDate.prototype.toTimeString = function () {
        ...
        var abbrev = abbr_timezone_1.abbrTimezone(this.timezone, this._utcDate);
        if (abbrev) {
            abbrev = " (" + abbrev + ")";
        }
