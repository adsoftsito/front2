...
export class IgxTreeGridRowComponent extends IgxRowComponent {
     ...
     */
    resolveClasses() {
        const /** @type {?} */ classes = super.resolveClasses();
        const /** @type {?} */ filteredClass = this.treeRow.isFilteredOutParent ? 'igx-grid__tr--filtered' : '';
     ...
     * @return {?}
     */
    hasDeletedParent(rowId) {
        if (this.grid.cascadeOnDelete) {
            const /** @type {?} */ node = this.grid.records.get(rowId);
            for (const /** @type {?} */ parentId of node.path) {
                const /** @type {?} */ state = this.grid.transactions.getState(parentId);
